...
{  
  ...
  uint32_t temp;
  while (ss >> temp)
    gArray.push_back (temp);

  if (appl == std::string ("all"))
    {
      for (uint32_t i = 0; i < apNum; ++i)
        {
          // std::vector<int>::iterator it = find(gArray.begin (), gArray.end (), i);
          // if (it == gArray.end ())
            aArray.push_back (i);
        }
    }
  ...
  else
    {
      ...
      if (rateControl == std::string ("ideal"))
        ...
        wifi.SetRemoteStationManager ("ns3::ConstantRateWifiManager",
                                      ...
                                      "DataMode", StringValue (constantRate),
                                      "RtsCtsThreshold", UintegerValue (99999)); // level changed 9->10 since tcp-1 exp
...
AodvExample::CreateCsmaDevices ()
{
  ...
  for (uint32_t i = 0; i < gArray.size (); ++i)
    csmaNodes.Add (apNodes.Get (gArray[i]));
...
AodvExample::InstallInternetStack ()
{
  ...
  olsr.Set ("TcInterval", TimeValue (Seconds (3)));
  olsr.Set ("MidInterval", TimeValue (Seconds (3)));
  olsr.Set ("HnaInterval", TimeValue (Seconds (3)));
...
AodvExample::InstallApplications ()
{
  ...
  if (app == std::string("udp"))
    {
      for (uint32_t j = 0; j < aArray.size(); ++j)
        {
          uint32_t i = aArray[j];
