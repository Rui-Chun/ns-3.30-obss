...
for caIndex = 1:length(caRange)
    for mcsIndex = 1:length(mcsRange)
        ...
        Y = squeeze(thr(1, caIndex, mcsIndex, :, :)); Y = nanmedian(Y.',1);
        plot(X, Y, ...
...
for caIndex = 1:length(caRange)
    for mcsIndex = 1:length(mcsRange)
        X = repmat(inRange * 1e2, length(rngRange), 1); X = nanmedian(X,1);
        Y = squeeze(delay(1, caIndex, mcsIndex, :, :)); Y = nanmedian(Y.',1);
        plot(X, Y, ...
            'LineStyle', lineRange{caIndex}, ...
            'Marker', markerRange{caIndex}, ...
            'MarkerSize', 6, ...
...
for caIndex = 1:length(caRange)
    for mcsIndex = 1:length(mcsRange)
        X = repmat(inRange * 1e2, length(rngRange), 1); X = nanmedian(X,1);
        Y = squeeze(loss(1, caIndex, mcsIndex, :, :)); Y = nanmedian(Y.',1);
        plot(X, Y, ...
            'LineStyle', lineRange{caIndex}, ...
            'Marker', markerRange{caIndex}, ...
            'MarkerSize', 6, ...
