...
do
    ./waf --run "mesh-loc-jw --rateControl=obss --datarateUp=false --gridSize=3 --apXStep=30 --apYStep=30 --RngSeed=${cnt} --startTime=10 --totalTime=40 --route=static --datarate=8e6 --mac=adhoc --app=udp --constantRate=HeMcs3 --apNum=9 --gateways=0+3+6 --appl=2+5+8 --recordPath=./obss-bash-udp-1/grid3o-${cnt}-record.txt" | tee ./obss-bash-udp-1/grid3o-${cnt}.txt
    # cp /home/jiaming/TransRecords.txt ./obss-bash/grid3o-${cnt}-record.txt
done
