 ...
 *
 * Trigger frames include one or more User Info fields, each of which
 * carries information about the HE TB PPDU that the addressed station
 * sends in response to the Trigger frame.
 */
...
public:
   ...
   *
   * \param bcc whether to use LDPC or not
   */
   ...
   * \param nSs the number of spatial stream (a value from 1 to 8)
   */
   ...
   * an HE TB PPDU response at its maximum transmit power for the assigned MCS
   */
  bool GetUlTargetRssiMaxTxPower (void) const;
  /**
   * Get the expected receive signal power for the solicited HE TB PPDU. This
   * method can only be called if GetUlTargetRssiMaxTxPower returns false.
   *
  ...
  /**
   * Get the Preferred AC. This method can only be called if this
   * User Info field is included in a Basic Trigger frame.
   *
   * \return the Preferred AC
   */
...
private:
  ...
  union
  {
    uint8_t basic;   //!< Basic Trigger variant
    // TODO BAR Control & BAR Information   //!< MU-BAR variant
  } m_triggerDependentUserInfo;
};
 ...
 *
 * 802.11ax amendment includes eight types of Trigger frames:
 *   - Basic
 ...
 * TODO For now only the Basic, MU-BAR and MU-RTS variants are supported.
...
public:
   ...
   *
   * \param sr the value for the UL Spatial Reuse
   */
  ...
  typedef std::list<CtrlTriggerUserInfoField>::const_iterator Iterator;
  ...
  /**
   * Get an iterator pointing to the first User Info field found (starting from
   * the one pointed to by the given iterator) whose AID12 subfield is set to
   * the given value.
   *
   * \param start an iterator pointing to the User Info field to start the search from
   * \param aid12 the value of the AID12 subfield to match
   * \return an iterator pointing to the User Info field matching the specified
   * criterium, if any, or an iterator indicating past-the-last User Info field.
   */
  Iterator FindUserInfoWithAid (Iterator start, uint16_t aid12) const;
  /**
   * Get an iterator pointing to the first User Info field found whose AID12
   ...
   * criterium, if any, or an iterator indicating past-the-last User Info field.
   */
  Iterator FindUserInfoWithAid (uint16_t aid12) const;
  /**
   * Get an iterator pointing to the first User Info field found (starting from
   ...
   * criterium, if any, or an iterator indicating past-the-last User Info field.
   */
  Iterator FindUserInfoWithRaRuAssociated (Iterator start) const;
  /**
   * Get an iterator pointing to the first User Info field found which allocates
   ...
   * criterium, if any, or an iterator indicating past-the-last User Info field.
   */
  Iterator FindUserInfoWithRaRuAssociated (void) const;
  /**
   * Get an iterator pointing to the first User Info field found (starting from
   * the one pointed to by the given iterator) which allocates a Random Access
   ...
   * criterium, if any, or an iterator indicating past-the-last User Info field.
   */
  Iterator FindUserInfoWithRaRuUnassociated (Iterator start) const;
  /**
   * Get an iterator pointing to the first User Info field found which allocates
   ...
   * criterium, if any, or an iterator indicating past-the-last User Info field.
   */
  Iterator FindUserInfoWithRaRuUnassociated (void) const;
  /**
   * Check the validity of this Trigger frame.
   * TODO Implement the checks listed in Section 27.5.3.2.3 (Allowed settings
   * of the Trigger frame fields and TRS Control subfield).
   *
...
private:
  ...
  uint8_t m_apTxPower;   //!< AP Tx Power
