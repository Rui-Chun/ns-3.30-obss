    ...
    m_triggerType (triggerType)
{
  ...
  memset (&m_triggerDependentUserInfo, 0, sizeof (m_triggerDependentUserInfo));
}
...
CtrlTriggerUserInfoField::GetSerializedSize (void) const
{
  ...
  case BFRP_TRIGGER:
    ...
    break;
  case MU_BAR_TRIGGER:
    // TODO size += ; // BAR Control and BAR Information
    break;
  }
...
CtrlTriggerUserInfoField::Serialize (Buffer::Iterator start) const
{
  ...
  NS_ABORT_MSG_IF (m_triggerType == MU_BAR_TRIGGER, "MU-BAR Trigger frame is not supported");
  ...
  if (m_triggerType == BASIC_TRIGGER)
    {
      i.WriteU8 (m_triggerDependentUserInfo.basic);
    }
...
CtrlTriggerUserInfoField::Deserialize (Buffer::Iterator start)
{
  ...
  m_ulTargetRssi = i.ReadU8 () & 0x7f;

  if (m_triggerType == BASIC_TRIGGER)
    {
      m_triggerDependentUserInfo.basic = i.ReadU8 ();
    }
...
bool
CtrlTriggerUserInfoField::GetUlTargetRssiMaxTxPower (void) const
{
...
CtrlTriggerUserInfoField::SetBasicTriggerDepUserInfo (uint8_t spacingFactor, uint8_t tidLimit, AcIndex prefAc)
{
  ...
  m_triggerDependentUserInfo.basic = (spacingFactor & 0x03)
                                     | (tidLimit & 0x07) << 2
                                     | (prefAc & 0x03) << 6;
}
...
CtrlTriggerUserInfoField::GetMpduMuSpacingFactor (void) const
{
  ...
  return m_triggerDependentUserInfo.basic & 0x03;  
}
...
CtrlTriggerUserInfoField::GetTidAggregationLimit (void) const
{
  ...
  return (m_triggerDependentUserInfo.basic & 0x1c) >> 2;
}
...
CtrlTriggerUserInfoField::GetPreferredAc (void) const
{
  ...
  return AcIndex ((m_triggerDependentUserInfo.basic & 0xc0) >> 6);
}
  ...
  while (!isPadding)
    {
      if (i.ReadU16 () == 0xffff)
        ...
        {
          // go back 2 bytes and deserialize the User Info field
          i.Prev (2);
...
CtrlTriggerHeader::Iterator
CtrlTriggerHeader::FindUserInfoWithAid (Iterator start, uint16_t aid12) const
{
...
CtrlTriggerHeader::Iterator
CtrlTriggerHeader::FindUserInfoWithRaRuAssociated (Iterator start) const
{
...
{
  return FindUserInfoWithAid (m_userInfoFields.begin (), 0);
}

CtrlTriggerHeader::Iterator
CtrlTriggerHeader::FindUserInfoWithRaRuUnassociated (Iterator start) const
{
...
{
  return FindUserInfoWithAid (m_userInfoFields.begin (), 2045);
}
...
CtrlTriggerHeader::IsValid (void) const
{
  ...
  for (auto& ui : m_userInfoFields)
    {
      if (HeRu::Overlap (GetUlBandwidth (), ui.GetRuAllocation (), prevRus))
        {
          return false;
        }
