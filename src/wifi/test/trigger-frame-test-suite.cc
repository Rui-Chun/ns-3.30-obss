...
TriggerFrameSerialization::DoRun (void)
{
  ...
  HeRu::RuType ru2 = HeRu::RU_2x996_TONE;
  std::size_t index2 = 1;
  bool ldpc2 = false;
  uint8_t mcs2 = 11;
  bool ulDcm2 = true;
  ...
  NS_TEST_EXPECT_MSG_EQ (trigger.IsBasic (), true,
                         "Different trigger frame type found");
  ...
  NS_TEST_EXPECT_MSG_EQ (it->GetTidAggregationLimit (), tidLimitCopy,
  ...
  NS_TEST_EXPECT_MSG_EQ (it->GetUlTargetRssiMaxTxPower (), true,
                         "Different UL Target RSSI found");
  ...
  Buffer buffer;
  buffer.AddAtStart (trigger.GetSerializedSize ());
  trigger.Serialize (buffer.Begin ());

  CtrlTriggerHeader triggerCopy;
  triggerCopy.Deserialize (buffer.Begin ());

  // Check that the header has been correctly deserialized
  NS_TEST_EXPECT_MSG_EQ (triggerCopy.IsBasic (), true,
                         "Different trigger frame type found in deserialized header");
  ...
  CtrlTriggerHeader::Iterator itCopy = triggerCopy.FindUserInfoWithRaRuAssociated ();
  ...
  NS_TEST_EXPECT_MSG_EQ (itCopy->GetTidAggregationLimit (), tidLimitCopy,
  ...
  NS_TEST_EXPECT_MSG_EQ (itCopy->GetUlTargetRssiMaxTxPower (), true,
                         "Different UL Target RSSI found in deserialized header");
...
RuAllocationOverlap::DoRun (void)
{
   ...
   */
  for (auto& ru : HeRu::m_heRuSubcarrierRanges)
    {
      CtrlTriggerHeader trigger;
      trigger.SetType (BASIC_TRIGGER);
   ...
   * RU_52_TONE (15,16), RU_26_TONE (33)
   */
  {
    CtrlTriggerHeader trigger;
      trigger.SetType (BASIC_TRIGGER);
...
TriggerFrameTestSuite::TriggerFrameTestSuite ()
  : TestSuite ("trigger-frame", UNIT)
{
  AddTestCase (new TriggerFrameSerialization, TestCase::QUICK);
