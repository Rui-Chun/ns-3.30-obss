 ...
 *
 * The POTENTIAL protocol (\RFC{2453}) is a unicast-only IPv4 IGP (Interior Gateway Protocol).
 ...
 * data flows.
 *
 * POTENTIAL implements the Bellman-Ford algorithm (although the RFC does not state it).
 ...
 *
 * For the POTENTIAL protocol, the exact convergence time is shorter, thanks to the
...
/**
 * \ingroup potential
 * \brief Potential Routing Table Entry
 */
class PotentialRoutingTableEntry : public Ipv4RoutingTableEntry
{
public:
  ...
  PotentialRoutingTableEntry (void);
   ...
   */
  PotentialRoutingTableEntry (Ipv4Address network, Ipv4Mask networkPrefix, Ipv4Address nextHop, uint32_t interface);
   ...
   * \param interface interface index
   */
  PotentialRoutingTableEntry (Ipv4Address network, Ipv4Mask networkPrefix, uint32_t interface);

  virtual ~PotentialRoutingTableEntry ();
 ...
 */
std::ostream& operator<< (std::ostream& os, PotentialRoutingTableEntry const& route);
 ...
 * \ingroup potential
 *
 * \brief POTENTIAL Routing Protocol, defined in \RFC{2453}.
 */
class Potential : public Ipv4RoutingProtocol
{
public:
  // /< C-tor
  Potential ();
...
private:
  /// Container for the network routes - pair PotentialRoutingTableEntry *, EventId (update event)
  typedef std::list<std::pair <PotentialRoutingTableEntry *, EventId> > Routes;

  /// Const Iterator for container for the network routes
  typedef std::list<std::pair <PotentialRoutingTableEntry *, EventId> >::const_iterator RoutesCI;

  /// Iterator for container for the network routes
  typedef std::list<std::pair <PotentialRoutingTableEntry *, EventId> >::iterator RoutesI;


  /**
   * \brief Receive POTENTIAL packets.
   *
  ...
  /**
   * \brief Handle POTENTIAL requests.
   *
   ...
   */
  void HandleRequests (PotentialHeader hdr, Ipv4Address senderAddress, uint16_t senderPort, uint32_t incomingInterface, uint8_t hopLimit);

  /**
   * \brief Handle POTENTIAL responses.
   *
   * \param hdr message header (including RTEs)
   * \param senderAddress sender address
   * \param incomingInterface incoming interface
   * \param hopLimit packet's hop limit
   */
  void HandleResponses (PotentialHeader hdr, Ipv4Address senderAddress, uint32_t incomingInterface, uint8_t hopLimit);
   ...
   */
  void RecvUnicastPotential (Ptr<Socket> socket);
  /**
   * Receive and process multicast packet
   * \param socket socket where packet is arrived
   */
  void RecvMulticastPotential (Ptr<Socket> socket);
   ...
   */
  void InvalidateRoute (PotentialRoutingTableEntry *route);
